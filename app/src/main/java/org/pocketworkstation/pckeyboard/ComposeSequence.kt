...
open class ComposeSequence(
    ...
    companion object {
        ...
        private fun showString(input: String): String =
            "$input{${input.toList().joinToString(",") { it.code.toString() }}}"
        ...
        protected fun format(seq: String): String = buildString {
            var quoted = false
            for (c in seq) {
                val name = keyNames[c.code]
                if (name != null) {
                    append(if (quoted) "\" " else " ")
                    append(name)
                    quoted = false
                } else {
                    if (!quoted) append(if (isNotEmpty()) " \"" else "\"")
                    if (c < ' ' || c == '"' || c == '\\')
                        append("\\").append(if (c < ' ') (c + 64).toChar() else c)
                    else
                        append(c)
                    quoted = true
                }
        ...
        protected fun put(key: String, value: String) {
            ...
            when {
                key in mMap -> Log.w(TAG, "compose sequence is a duplicate: ${format(key)}")
                key in mPrefixes -> Log.w(TAG, "compose sequence is a subset: ${format(key)}")
            }
